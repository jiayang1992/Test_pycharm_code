...
class Re_import_data(unittest.TestCase):
    ...
    def test_(self):
        ...
        driver.find_element_by_name("ids").click()
        driver.find_element_by_xpath("//div[@onclick='importAgain(1,1)']").click()
        self.assertEqual(u"你确定导入这些信息吗？", self.close_alert_and_get_its_text())
        time.sleep(3)
        self.assertEqual(u"导入成功", self.close_alert_and_get_its_text())
        driver.find_element_by_css_selector("div.fl-l.lead-btn").click()
        Select(driver.find_element_by_id("regOrgId")).select_by_value("1508901692722455")
        driver.find_element_by_name("enterpriseName").send_keys("延长石油")
