...
class Case(unittest.TestCase):
    ...
    def test_case(self):
        ...
        Select(driver.find_element_by_name("isEnabled")).select_by_visible_text(u"是")
        time.sleep(3)
        driver.find_element_by_id("queryBtn").click()
        value = driver.find_element_by_name("totalNum").get_attribute("value")
        print("以是否显示'是'搜索，搜索到 %d 条结果" % int(value))
        driver.find_element_by_xpath(u"//button[@value='重置']").click()
        Select(driver.find_element_by_name("isEnabled")).select_by_visible_text(u"否")
        time.sleep(3)
        driver.find_element_by_id("queryBtn").click()
        value = driver.find_element_by_name("totalNum").get_attribute("value")
        print("以是否显示'否'搜索，搜索到 %d 条结果" % int(value))
        driver.find_element_by_xpath(u"//button[@value='重置']").click()
